...
total=`wc -l ${filename}`
sentmail=`grep "status=sent" /var/log/maillog | grep  -v "axonadviser" | wc -l`
...
echo >> ${filename}
echo "Total Duplicate Emails - ${total}" >> ${filename}
        
#test email for me only
echo "This is an automated report. Please see the attached txt file to check if duplicate emails are sent,if file is blank then no duplicates are sent. Duplicate emails found total:${total}" | /usr/bin/mutt -a "${filename}" -e "my_hdr From:system@axonadvisers.com" -s "No reply: Duplicate Summary. TotalEmailsSent : ${sentmail} " --  urgen@gmail.com,sherpa@gmail.com
